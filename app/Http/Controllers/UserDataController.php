...
class UserDataController extends Controller
{
    ...
    public function handleUsersVehicleData(Request $request, $action, $id = null)
    {
        switch ($action) {
            case 'view':
                // $user_data = DB::connection('sarvisolution_app')->table('vehicle_data')->get();4
                $user_data = VehicleData::all();
            ...
            case 'add':
                if ($request->isMethod('post')) {
                    ...
                    $user = DB::connection('sarvisolution_app')->table('register_form')->where('id', $request->input('new_user_id'))->first();
                    ...
                    VehicleData::create([
                        ...
                        'created_at' => now(), // Store current Unix timestamp
                        'updated_at' => now(), // Store current Unix timestamp
                    ]);
                    
                        return back()->with('status_message', 'Added Successfully!');
                    } else {
                        // Handle GET request - Show form
                        $vehicles_name = DB::connection('sarvisolution_app')->table('vehicle_data')->select('vehicle_name')->get();
                ...
                case 'delete':
                    if ($id) {
                        DB::connection('sarvisolution_app')->table('vehicle_data')->where('id', $id)->delete();
    ...
    public function checkUserId(Request $request)
    {
        ...
        $user = DB::connection('sarvisolution_app')->table('register_form')->where('id', $new_user_id)->first();
